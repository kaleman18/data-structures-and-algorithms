...
const addOne = (arr) => {
  let newArray = [];
  arr.forEach((v) => newArray.push(v + 1));
  return newArray;
};
...
const allUpperCase = (arr) => {
  let newArray = [];
  arr.forEach((v) => newArray.push(v.toUpperCase()));
  return newArray;
};
...
const addValues = (arr, value) => {
  // Solution code here...
  let newArray = [];
  arr.forEach((v) => newArray.push(v + value));
  return newArray;
};

const addNumbers = (num, arr, times, callback) => {
  // did not finish got to one hour!
